import { fetchSystems, fetchHumanDecisionStats, fetchHumanDecisionsPage } from '@/lib/data'
import { HumanOversightClient } from './HumanOversightClient'

export const dynamic = 'force-dynamic'

export default async function OversightPage() {
  const [systems, stats, firstPage] = await Promise.all([
    fetchSystems(),
    fetchHumanDecisionStats(),
    fetchHumanDecisionsPage(0, 50),
  ])

  const highRiskSystems = systems.filter((s: any) => s.risk === 'High')
  const overrideRate = stats.total > 0 ? Math.round((stats.overrides / stats.total) * 100) : 0
  ...
  return (
    <div className="space-y-8 fade-in">
      ...
      <div className="card !bg-coal2 !border-ash p-6">
        <div className="flex items-start gap-4">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(184,115,82,0.15)', border: '1px solid rgba(184,115,82,0.3)' }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--bronze)" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M3 6h18M3 12h18M3 18h18"/></svg>
          </div>
          <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Oversight logs', value: stats.total.toLocaleString(), sub: 'total recorded' },
