...
function EvidenceRecordButton({ systems }: { systems: System[] }) {
  ...
  function generate() {
    ...
    window.open(`/api/oversight-evidence-pdf?${params.toString()}`, '_blank')
  ...
  const modal = open ? (
    <div className="theme-dark" style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }} onClick={() => setOpen(false)}>
      <div aria-hidden style={{ position: 'absolute', inset: 0, background: 'rgba(7,14,28,0.88)', backdropFilter: 'blur(6px)' }} />
      <div onClick={e => e.stopPropagation()} style={{ position: 'relative', width: '100%', maxWidth: 500, borderRadius: 18, padding: 28, background: 'var(--coal2)', border: '1px solid var(--ash)', boxShadow: '0 24px 60px -20px rgba(0,0,0,0.7)' }}>
        <div className="flex items-start justify-between mb-5">
          <div>
            <span className="eyebrow !text-bronze text-[10px]">Art. 14 · EU AI Act</span>
            <h2 className="display-serif text-cream mt-2" style={{ fontSize: '1.4rem', fontWeight: 400 }}>
              Generate <span className="italic font-light text-bronze">evidence record</span>
            </h2>
            <p className="text-xs text-cream2/60 mt-1.5 font-light leading-relaxed">
              Produces a court-admissible PDF document containing the full oversight log, all override decisions with reasons, and a reviewer sign-off block. Print or save as PDF from your browser.
            </p>
          </div>
          <button onClick={() => setOpen(false)} className="text-cream2/60 hover:text-cream text-2xl w-8 h-8 flex items-center justify-center rounded-full hover:bg-coal3">×</button>
        </div>

        <div className="space-y-4">
          <div>
            <label className="eyebrow !text-cream2/60 block mb-2">AI system (optional — leave blank for all)</label>
            <select value={systemId} onChange={e => setSystemId(e.target.value)} className="w-full bg-coal border border-ash rounded-xl px-4 py-3 text-sm text-cream focus:outline-none focus:border-bronze transition">
              <option value="">All systems</option>
              {systems.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
            </select>
          </div>
          <div>
            <label className="eyebrow !text-cream2/60 block mb-2">Reviewer name</label>
            <input value={reviewerName} onChange={e => setReviewerName(e.target.value)} placeholder="e.g. Sarah Murphy, HR Director" className="w-full bg-coal border border-ash rounded-xl px-4 py-3 text-sm text-cream placeholder:text-cream2/40 focus:outline-none focus:border-bronze transition" />
          </div>
          <div>
            <label className="eyebrow !text-cream2/60 block mb-2">Cycle label</label>
            <input value={cycle} onChange={e => setCycle(e.target.value)} placeholder="e.g. Admissions 2026 — Round 1 and 2" className="w-full bg-coal border border-ash rounded-xl px-4 py-3 text-sm text-cream placeholder:text-cream2/40 focus:outline-none focus:border-bronze transition" />
          </div>

          <div className="rounded-xl p-3 text-xs" style={{ background: 'rgba(74,144,217,0.05)', border: '1px solid rgba(74,144,217,0.15)' }}>
            <div className="text-glacier-light font-medium mb-1">What this document contains</div>
            <div className="text-cream2/60 space-y-0.5">
              <div>· AI system details and risk classification</div>
              <div>· Full decision summary with override rate</div>
              <div>· Every override with written reason</div>
              <div>· All manual oversight entries</div>
              <div>· Complete decision log</div>
              <div>· Data integrity statement</div>
              <div>· Reviewer signature block</div>
            </div>
          </div>

          <div className="flex items-center justify-end gap-2 pt-2">
            <button onClick={() => setOpen(false)} className="btn-ghost !py-2 !px-4 text-[13px]">Cancel</button>
            <button
              onClick={generate}
              disabled={generating}
              className="flex items-center gap-2 text-[13px] px-5 py-2.5 rounded-xl transition disabled:opacity-60"
              style={{ background: 'linear-gradient(135deg, #1a3a6b 0%, #2563b0 100%)', color: 'white', fontWeight: 600 }}
            >
              ...
              {generating ? 'Opening...' : 'Generate evidence record'}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <>
      <button
        ...
        style={{ background: 'linear-gradient(135deg, rgba(15,34,64,0.6) 0%, rgba(37,99,176,0.15) 100%)', border: '1px solid rgba(74,144,217,0.35)', color: 'var(--glacier-light)', fontWeight: 500 }}
      >
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {decisions.length > 0 && (
            <>
              ...
              <EvidenceRecordButton systems={systems} />
            </>
          )}
